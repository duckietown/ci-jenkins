...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
    ...
    exec java -DJENKINS_HOME="$JENKINS_HOME" -Duser.home="$JENKINS_HOME" -Dhudson.model.ParametersAction.safeParameters=PARENT_JOB_FAILED "${java_opts_array[@]}" -jar ${JENKINS_WAR} "${jenkins_opts_array[@]}" "$@"
fi
